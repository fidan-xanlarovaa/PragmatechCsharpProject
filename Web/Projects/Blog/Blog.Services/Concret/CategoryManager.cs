...
namespace Blog.Services.Concret
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var entities = await _unitOfWork.Categories.GetAllAsync(c => !c.IsDeleted); // !c.IsDeleted  => c.IsDeleted==false demekdir
        ...
        public async Task<IDataResult<CategoryDto>> AddAsync(CategoryAddDto dto, string createdByName)
        {
            ...
            var addedEntity = await _unitOfWork.Categories.AddAsync(entity); // bu qizarirdi cunki evvel  Task AddAsync(T entity); idi ve geriye task qaytarirdi
                                                                             // bize ise entitu lazim olduqu ucun  Task<T> AddAsync(T entity); yazdiqdan sonra error
                                                                             // arardan qalxir
        ...
        public async Task<IDataResult<CategoryDto>> UpdateAsync(CategoryUpdateDto dto, string createdByName)
        {
            ...
            var entity = _mapper.Map<CategoryUpdateDto, Category>(dto, dbEntity); // gelen updated data da bezi melumatlar yoxdur (createdby name ve s.)
                                                                                  // o datalari null etmesin deye bu hisseni yaziriq
